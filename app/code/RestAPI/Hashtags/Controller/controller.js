...
import { HashtagList, Hashtag } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const hashtagList = new HashtagList(dbConn.module);
    await hashtagList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await hashtagList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const hashtagReturnData = await hashtagList?.getDataByID(id);
          if (!hashtagReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await hashtagReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newHashtag = new Hashtag(dbConn.module);
        const objReturn = await newHashtag.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await hashtagList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await hashtagList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
