...
import { ConversationParticipantList, ConversationParticipant } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const conversationParticipantList = new ConversationParticipantList(dbConn.module);
    await conversationParticipantList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await conversationParticipantList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const conversationParticipantReturnData = await conversationParticipantList?.getDataByID(id);
          if (!conversationParticipantReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await conversationParticipantReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newConversationParticipant = new ConversationParticipant(dbConn.module);
        const objReturn = await newConversationParticipant.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await conversationParticipantList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await conversationParticipantList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
