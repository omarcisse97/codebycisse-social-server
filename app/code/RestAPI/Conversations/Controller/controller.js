...
import { ConversationList, Conversation } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const conversationList = new ConversationList(dbConn.module);
    ...
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await conversationList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const conversationReturnData = await conversationList?.getDataByID(id);
          if (!conversationReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await conversationReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        const newConversation = new Conversation(dbConn.module);
        const objReturn = await newConversation.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await conversationList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await conversationList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
