...
import { PostList, Post } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const postList = new PostList(dbConn.module);
    await postList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await postList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const postReturnData = await postList?.getDataByID(id);
          if (!postReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await postReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newPost = new Post(dbConn.module);
        const objReturn = await newPost.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await postList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await postList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
