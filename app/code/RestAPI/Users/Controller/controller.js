...
import { UserList, User } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    if ((method === 'GET' || method === 'PUT' || method === 'DELETE')  && (!params || !params?.handle)) {
    ...
    if (!request?.body && method === 'POST') {
      throw new Error('Missing crucial body');
    }
    if (!headers?.api_key) {
    ...
    if (!id && (method === 'GET' || method === 'PUT' || method === 'DELETE') ) {
      throw new Error('Missing crucial param to update or retrieve data');
    }

    const userList = new UserList(dbConn.module);
    await userList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await userList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const userReturnData = await userList?.getDataByID(id);
          // if (!userReturnData) {
          //   throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          // }
          const retValFetch = userReturnData? await userReturnData?.getData() : {};
          console.log('Returning retVal -> ', retValFetch);
          return resolution?.json({
            success: true,
            data: retValFetch,
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newUser = new User(dbConn.module);
        const objReturn = await newUser.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await userList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await userList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
