...
import { ReportList, Report } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const reportList = new ReportList(dbConn.module);
    await reportList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await reportList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const reportReturnData = await reportList?.getDataByID(id);
          if (!reportReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await reportReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newReport = new Report(dbConn.module);
        const objReturn = await newReport.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await reportList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await reportList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
