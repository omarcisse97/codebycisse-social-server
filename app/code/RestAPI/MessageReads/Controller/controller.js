...
import { MessageReadList, MessageRead } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const messageReadList = new MessageReadList(dbConn.module);
    await messageReadList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await messageReadList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const messageReadReturnData = await messageReadList?.getDataByID(id);
          if (!messageReadReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await messageReadReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newMessageRead = new MessageRead(dbConn.module);
        const objReturn = await newMessageRead.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await messageReadList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await messageReadList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
