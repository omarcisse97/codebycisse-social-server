...
import { MessageList, Message } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const messageList = new MessageList(dbConn.module);
    ...
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await messageList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const messageReturnData = await messageList?.getDataByID(id);
          if (!messageReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await messageReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newMessage = new Message(dbConn.module);
        const objReturn = await newMessage.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await messageList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await messageList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
