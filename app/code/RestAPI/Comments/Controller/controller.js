...
import { CommentList, Comment } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const commentList = new CommentList(dbConn.module);
    await commentList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await commentList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const commentReturnData = await commentList?.getDataByID(id);
          if (!commentReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await commentReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newComment = new Comment(dbConn.module);
        const objReturn = await newComment.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await commentList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await commentList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
