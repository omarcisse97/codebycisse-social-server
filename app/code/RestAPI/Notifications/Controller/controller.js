...
import { NotificationList, Notification } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const notificationList = new NotificationList(dbConn.module);
    ...
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await notificationList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const notificationReturnData = await notificationList?.getDataByID(id);
          if (!notificationReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await notificationReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newNotification = new Notification(dbConn.module);
        const objReturn = await newNotification.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await notificationList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await notificationList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
