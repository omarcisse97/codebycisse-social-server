...
import { PostHashtagList, PostHashtag } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const postHashtagList = new PostHashtagList(dbConn.module);
    await postHashtagList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await postHashtagList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const postHashtagReturnData = await postHashtagList?.getDataBySNUM(id);
          if (!postHashtagReturnData) {
            throw new Error(`Data not found. Invalid SNUM "${body?.id}"`);
          }
          const retValFetch = await postHashtagReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newPostHashtag = new PostHashtag(dbConn.module);
        const objReturn = await newPostHashtag.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await postHashtagList?.getDataBySNUM(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await postHashtagList?.getDataBySNUM(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
