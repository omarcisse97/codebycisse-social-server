...
import { UserSettingList, UserSetting } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const userSettingList = new UserSettingList(dbConn.module);
    await userSettingList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await userSettingList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const userSettingReturnData = await userSettingList?.getDataByID(id);
          if (!userSettingReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await userSettingReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newUserSetting = new UserSetting(dbConn.module);
        const objReturn = await newUserSetting.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await userSettingList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await userSettingList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
