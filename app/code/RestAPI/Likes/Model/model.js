export class Like {
    ...
    async init(id = null) {
        try {
            ...
            this._id = currentId;
            this._user_id = likeData.user_id;
            this._post_id = likeData.post_id;
            this._comment_id = likeData.comment_id;
            this._created_at = likeData.created_at;
        } catch (error) {
            console.error('Failed to initialize Like model. Error(s)', error);
        }
    }

    async getData() {
        try {
            if (!this._id || !this._user_id || (!this._post_id && !this._comment_id)) {
    ...
    async create(data) {
        try {
            ...
            const validFields = ['user_id', 'post_id', 'comment_id'];
    ...
    async delete() {
        try {
            ...
            const result = await this._dbConn.client.query(`DELETE FROM likes WHERE id = $1 RETURNING *`, [this._id]);
...
export class LikeList {
    ...
    async init() {
        try {
            ...
            const result = await this.dbConn.fetch('SELECT * FROM likes');
            if (!result?.data || !Array.isArray(result?.data)) {
                throw new Error('SQL error occurred when fetching data');
            }
            result?.data?.map((row) => {
                this.data.push(new Like(this.dbConn, row?.id, row?.user_id, row?.post_id, row?.comment_id, row?.created_at));
            });
        } catch (error) {
            console.error('LikeList init failed. Error(s): ', error);
        }
    }
    ...
    async getAllData() {
        try {
            ...
            const retVal = [];
            for (const like of this.data) {
                retVal.push(await like?.getData());
            }
            return retVal;
        } catch (error) {
