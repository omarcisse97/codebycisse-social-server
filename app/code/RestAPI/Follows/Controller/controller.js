...
import { FollowList, Follow } from "../Model/model.js";
...
const Controller = async (request = null, resolution = null) => {
  try {
    ...
    const followList = new FollowList(dbConn.module);
    await followList.init();
    switch (method) {
      case 'GET':
        ...
        if (id === 'all') {
          const result = await followList?.getAllData();
          let retVal = [];
          if(body?.limit){
          ...
          const followReturnData = await followList?.getDataByID(id);
          if (!followReturnData) {
            throw new Error(`Data not found. Invalid ID "${body?.id}"`);
          }
          const retValFetch = await followReturnData?.getData();
          return resolution?.json({
            success: true,
            data: retValFetch ?? {},
            error: ''
      ...
      case 'POST':
        ...
        // }
        const newFollow = new Follow(dbConn.module);
        const objReturn = await newFollow.create(body?.data);
        if (!objReturn) {
      ...
      case 'PUT':
        ...
        const tmpSup = await followList?.getDataByID(id);
        if (!tmpSup) {
      ...
      case 'DELETE':
        ...
        const tmpDel = await followList?.getDataByID(id);
        if (!tmpDel) {
          throw new Error('Record not found');
        }
